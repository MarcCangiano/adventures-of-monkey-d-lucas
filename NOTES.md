...
- Flick training: cutthroats pop up in rowboats on a big perspective sea
  ...
  56*sc) to drop him. THREE TARGETS up at all times (Boss iterated 1 -> 3 same day): a replacement spawns the frame one drops, spaced off the others and off the last kill. NOBODY ATTACKS + NOTHING EXPIRES (Boss): pirates persist until SHOT (no windows, no glow telegraph), boss has no volleys, hearts gone — the only fail state is the boss 30s timeout. HUD has no level counter and no top-right bar; boss pace lurches randomly (spdTarget 0.35-2.5 rerolled every 0.5-1.7s, eased) on a higher base (1.5+0.42i). LEVEL = TIMERS not kill counts: 60s pop-up gauntlet, then a 60s BOSS TRACKING exercise — the boss NEVER DIES and has NO health/track bar (Boss removed it); score = time-on-target (B.onT), shown live in HUD and reported per level + averaged on the win screen. NO fail states at all now. Between phases: #bossready confirm overlay (game fully paused, S.bossWait) — boss only starts on the Face-him click. (TTS laugh removed; Boss wants the Envato Elements "pirate-laugh-SUG5NVH" instead — NO Envato creds on this Mac, so HE must download it and save as music/boss-laugh.mp3; playLaugh() no-ops until the file exists, then fires every 8-14s in boss fights at 0.5 vol.) Gun is silent — SFX only on hits. Window 2.4-0.09i,
...
(value=avg tracking pct). NAME IS REQUIRED UP FRONT: title-screen input gates Set sail (disabled until non-empty; mdl-name remembered); a finished 10-sea run AUTO-SUBMITS to both boards under that name (hs-status line reports carved/unreachable). No carve button anymore. QUIT: Esc or the strike-colors control (bottom-left, click rect x<210 y>H-40) ends the run on the results screen and carves partial stats (kills always; boss board only if at least one boss fight has 3s+ elapsed; TOT.seas counts completed seas for the tracking average). Seed entry: Capt Lucas.
